...
export const routePlanner = (state: typeof SupervisorState.State) => {
  ...
  // CRITICAL FIX: Find the last message from the planner agent, not just the last message
  // Progress messages and other agent messages should be skipped
  const plannerMessages = Array.isArray(messages) 
    ? messages.filter((m: any) => m.agent === 'planner') 
    : [];
  const lastPlannerMessage = plannerMessages.length > 0 
    ? plannerMessages[plannerMessages.length - 1] 
    : (Array.isArray(messages) && messages.length > 0 ? messages[messages.length - 1] : null);
  
  const lastMessage = lastPlannerMessage;
...
async function supervisor(state: typeof SupervisorState.State) {
  ...
  if (Array.isArray(messages)) {
    ...
    if (userMessages.length > 0) {
      ...
      const content = typeof latestUser.message?.content === 'string' 
        ...
        : JSON.stringify(latestUser.message?.content);
  ...
  // CRITICAL FIX: Find the last actual USER message, not just the last message
  // Progress messages and planner messages should be skipped
  const userMessages = Array.isArray(messages) ? messages.filter((m: any) => m.role === 'user') : [];
  const lastUserMessage = userMessages.length > 0 ? userMessages[userMessages.length - 1] : lastAnnotated;
  
  const messageContent = lastUserMessage && lastUserMessage.message
  ...
  console.log(`[supervisor] Pending product value:`, JSON.stringify(pendingProduct));
  console.log(`[supervisor] Planner recommendation:`, JSON.stringify(plannerRecommendation));
  console.log(`[supervisor] Last user message content: "${messageContent}"`);
  console.log(`[supervisor] EARLY CHECK CONDITIONS: workflowContext === 'awaiting_deal_confirmation': ${workflowContext === 'awaiting_deal_confirmation'}, pendingProduct exists: ${!!pendingProduct}`);
  ...
  if (workflowContext === 'awaiting_deal_confirmation' && pendingProduct) {
    console.log(`[supervisor] EARLY CHECK: Detected awaiting_deal_confirmation context, checking for user confirmation`);
    console.log(`[supervisor] EARLY CHECK: Planner recommendation:`, plannerRecommendation);
    
    // ENHANCED: Use planner's autoApplyIntent if available, otherwise fall back to keyword matching
    let hasAffirmative = false;
    
    if (plannerRecommendation && typeof plannerRecommendation.autoApplyIntent === 'boolean') {
      // Trust the planner's LLM-based intent analysis
      hasAffirmative = plannerRecommendation.autoApplyIntent;
      console.log(`[supervisor] EARLY CHECK: Using planner's autoApplyIntent: ${hasAffirmative}`);
    } else {
      // Fallback: Robust affirmative detection using keyword matching
      console.log(`[supervisor] EARLY CHECK: No planner recommendation, using keyword matching fallback`);
      ...
      hasAffirmative = affirmativePatterns.some(pattern => 
    ...
    if (hasAffirmative) {
      ...
      return {
        ...
        cartData,
        plannerRecommendation, // CRITICAL: Preserve planner recommendation even when clearing
        next: END
      };
    }
  }
...
async function catalogNode(state: typeof SupervisorState.State) {
  ...
  console.log('[catalogNode] === CATALOG AGENT PROCESSING ===');
  console.log('[catalogNode] User:', userId, 'Conversation:', conversationId);
  
  // Log user messages being processed
  if (Array.isArray(messages)) {
    const userMessages = messages.filter(m => m.role === 'user');
    console.log('[catalogNode] Processing', userMessages.length, 'user messages');
    if (userMessages.length > 0) {
      const latestUser = userMessages[userMessages.length - 1];
      const content = typeof latestUser.message?.content === 'string' 
        ? latestUser.message.content 
        : JSON.stringify(latestUser.message?.content);
      console.log('[catalogNode] Latest user message:', content.substring(0, 200));
    }
  }
  ...
  console.log('[catalogNode] Message content to send to catalog agent:', messageContent.substring(0, 200));
...
export async function cartAndCheckoutNode(state: typeof SupervisorState.State) {
  ...
  console.log('[cartAndCheckoutNode] All messages:', safeMessages.map(m => ({ 
    role: m.role, 
    content: m.message && m.message.content 
      ? (typeof m.message.content === 'string' ? m.message.content : String(m.message.content)).substring(0, 100)
      : 'No content'
  })));
  ...
  // CRITICAL FIX: Find the last ACTUAL USER message, not progress/system messages
  // Progress messages have empty or system-generated content
  const userMessages = safeMessages.filter((m: any) => m.role === 'user' && m.message && m.message.content);
  const lastUserMessage = userMessages.length > 0 ? userMessages[userMessages.length - 1] : undefined;
  
  const lastAnnotated = safeMessages.length > 0 ? safeMessages[safeMessages.length - 1] : undefined;
  const originalContent = lastUserMessage && lastUserMessage.message && lastUserMessage.message.content
    ? (typeof lastUserMessage.message.content === 'string' ? lastUserMessage.message.content : String(lastUserMessage.message.content))
    : '';
  ...
  const actualUserContent = originalContent;
  
  // Check if this is a checkout request based on actual user intent, not planner routing
  // ENHANCED: More specific checkout detection to avoid false positives
  const isCheckoutRequest = (
    actualUserContent.toLowerCase().includes('checkout') || 
    actualUserContent.toLowerCase().includes('complete') && actualUserContent.toLowerCase().includes('order') ||
    actualUserContent.toLowerCase().includes('place') && actualUserContent.toLowerCase().includes('order') ||
    actualUserContent.toLowerCase().includes('buy now') ||
    actualUserContent.toLowerCase().includes('purchase now') ||
    workflowContext === 'process_checkout' ||
    workflowContext === 'prepare_checkout'
  );
  
  console.log('[cartAndCheckoutNode] User message:', actualUserContent);
  console.log('[cartAndCheckoutNode] Is checkout request:', isCheckoutRequest);
  ...
  if (workflowContext === 'add_to_cart_with_deals' && pendingProduct) {
    ...
    if (isAutomaticFlow) {
      ...
      // CRITICAL: Explicitly tell agent to ONLY add to cart, not checkout
      messageToAgent += `. DO NOT proceed to checkout - just add the items to the cart and confirm.`;
    }
  ...
  let filteredMessages = safeMessages;
  if (workflowContext === 'add_to_cart_with_deals' && pendingProduct) {
    ...
    filteredMessages = safeMessages.filter((m: any) => {
    ...
    console.log(`[cartAndCheckoutNode] Filtered messages: ${safeMessages.length} -> ${filteredMessages.length}`);
  }
  ...
  const responseContent = resultMessage && resultMessage.content ? (typeof resultMessage.content === 'string' ? resultMessage.content : '') : '';
  ...
  // ENHANCEMENT: Add clear success message for add_to_cart_with_deals workflow
  if (workflowContext === 'add_to_cart_with_deals' && pendingProduct) {
    console.log('[cartAndCheckoutNode] Adding success confirmation for add_to_cart_with_deals workflow');
    
    // Extract success confirmation from cart agent response or create one
    const addedSuccessfully = contentStr.toLowerCase().includes('added') || 
                             contentStr.toLowerCase().includes('successfully') ||
                             contentStr.toLowerCase().includes('cart');
    
    if (addedSuccessfully) {
      const successMessage = new AIMessage(
        `✅ Successfully added ${pendingProduct.quantity || 1} ${pendingProduct.product} to your cart${dealData?.applied ? ' with deal applied' : ''}!`
      );
      
      const messagesWithSuccess = [
        ...annotatedResponses,
        annotateMessage(successMessage, 'assistant', 'cart_and_checkout')
      ];
      
      return {
        messages: messagesWithSuccess,
        userId,
        conversationId,
        workflowContext: null, // Clear workflow - operation complete
        dealData,
        pendingProduct: null, // Clear after successful add
        cartData,
...
async function dealsNode(state: typeof SupervisorState.State) {
  ...
  if (!effectivePending) {
    ...
    for (let i = recentUserMessages.length - 1; i >= 0 && !effectivePending; i--) {
      ...
      if (!msg.message || !msg.message.content) {
        console.warn(`[dealsNode] Message ${i + 1} has no content, skipping`);
        continue;
      }
  ...
  const dealsContext = buildAgentContextMessage(safeMessages as AnnotatedMessage[], 'deals', messageToAgent);
  ...
  const responseContent = annotatedResponses.length > 0 && annotatedResponses[0]?.message?.content 
    ? (typeof annotatedResponses[0].message.content === 'string' ? annotatedResponses[0].message.content : 'No deals found')
    : 'No deals found';
  
  // Get the ORIGINAL user message to check for auto-apply intent
  const userMessages = safeMessages.filter(m => m.role === 'user');
  const originalUserMessage = userMessages.length > 0 && userMessages[userMessages.length - 1].message && userMessages[userMessages.length - 1].message.content
  ...
  console.log('[dealsNode] Planner recommendation:', JSON.stringify(plannerRecommendation));
  if (!plannerRecommendation) {
  ...
  console.log('[dealsNode] Requires confirmation:', requiresConfirmation);
  console.log('[dealsNode] Is complex workflow:', isComplexWorkflow);
  console.log('[dealsNode] Response content preview:', responseContent.substring(0, 200));
  
  // CRITICAL FIX: Check autoApplyIntent FIRST, before checking for confirmation prompts
  // If user explicitly said "just take any deals", we should auto-apply regardless of response wording
  if (autoApplyIntent && !responseContent.toLowerCase().includes('no current deals') && !responseContent.toLowerCase().includes('no deals available')) {
    // ALWAYS auto-proceed when autoApplyIntent is true and deals are available
    console.log('[dealsNode] AUTO-APPLY: User explicitly requested auto-apply - proceeding without confirmation');
    ...
    // CRITICAL: Replace the deals agent's confirmation question with an auto-apply confirmation
    // Extract deal info from response if available
    const dealInfo = responseContent.match(/\$[\d.]+\s*off|[\d]+%\s*off|deal price:?\s*\$[\d.]+/i);
    const dealSummary = dealInfo ? dealInfo[0] : 'available deals';
    
    const autoApplyMessage = new AIMessage(
      `✅ Found ${dealSummary} on ${(effectivePending || pendingProduct)?.product}! Auto-applying and adding to your cart...`
    );
    
    // Add auto-apply confirmation and progress message instead of the deals agent's question
    const messagesWithProgress = [
      annotateMessage(autoApplyMessage, 'assistant', 'deals'),
      createProgressMessage('🛒 Adding items to your cart with deals applied...', 'supervisor')
    ];
  ...
  if (requiresConfirmation || (isComplexWorkflow && !responseContent.toLowerCase().includes('no current deals') && !responseContent.toLowerCase().includes('no deals available'))) {
    // Deal found - requires manual confirmation (autoApplyIntent was false or not set)
    
    // Note: autoApplyIntent=true case is already handled above, so we only reach here for manual confirmation
    console.log('[dealsNode] Manual confirmation required - awaiting user response');
    console.log('[dealsNode] Requires confirmation:', requiresConfirmation);
    ...
    console.log('[dealsNode] ROUTING DECISION:', {
      fromAgent: 'deals',
      toAgent: END,
      reason: 'Manual confirmation required - ending workflow to await user response',
      state: { 
    ...
    // DEFAULT: Deals found but no special handling - just display and end
    // This handles cases where deals are shown without confirmation prompts
    console.log('[dealsNode] Deals displayed, ending workflow to allow user response');
    
    // Log routing decision for debugging
    console.log('[dealsNode] ROUTING DECISION:', {
      fromAgent: 'deals',
      toAgent: END,
      reason: 'Deals displayed without confirmation - ending workflow',
      state: { 
        workflowContext: null,
        pendingProduct: effectivePending,
        dealData: { displayed: true, type: 'deals_info' }
      }
    });
    ...
    return {
      ...
      pendingProduct: effectivePending, // Keep pending product in case user wants to act on it
      cartData,
      userId,
      conversationId,
      plannerRecommendation, // CRITICAL: Preserve planner recommendation
...
async function notificationAgent(state: typeof SupervisorState.State) {
  // Create type-compatible wrapper functions
  const compatAnnotateMessage = (message: any, role: string, agent?: string) => {
    return annotateMessage(message, role as AgentRole, agent);
  };
  
  return notificationAgentImpl(state as any, compatAnnotateMessage, createProgressMessage);
}
...
export class SupervisorAgent {
  ...
  // Generate thread ID for local graph execution
  ...
  async invoke(input: { messages: any[], conversationId?: string }, config?: any) {
    ...
    // Ensure incoming messages are annotated (handle both LangChain instances and plain objects)
    console.log('[SupervisorChatClient] === INCOMING MESSAGES ===');
    console.log('[SupervisorChatClient] Received', input.messages?.length || 0, 'messages');
    console.log('[SupervisorChatClient] Raw input:', JSON.stringify(input, null, 2));
    if (input.messages && input.messages.length > 0) {
      input.messages.forEach((m, idx) => {
        console.log(`[SupervisorChatClient] Message ${idx} full structure:`, JSON.stringify(m, null, 2));
      });
    }
    
    const annotatedMessages = input.messages.map((m, idx) => {
      // Already an AnnotatedMessage
      if ((m as AnnotatedMessage).message) {
        return m as AnnotatedMessage;
      }
      
      // Handle LangChain message instances
      ...
      if (m instanceof AIMessage) {
        return annotateMessage(m, 'assistant');
      }
      
      // Handle plain objects from LangGraph client
      if (m && typeof m === 'object') {
        let messageInstance;
        
        if (m.type === 'human' || (m.role === 'user' || m.role === 'human')) {
          messageInstance = new HumanMessage(m.content || m.message || '');
          return annotateMessage(messageInstance, 'user');
        }
        
        if (m.type === 'system' || m.role === 'system') {
          messageInstance = new SystemMessage(m.content || m.message || '');
          return annotateMessage(messageInstance, 'system');
        }
        
        if (m.type === 'ai' || m.role === 'assistant' || m.role === 'ai') {
          messageInstance = new AIMessage(m.content || m.message || '');
          return annotateMessage(messageInstance, 'assistant');
        }
        
        // Fallback: treat as human message if it has content
        if (m.content) {
          messageInstance = new HumanMessage(m.content);
          return annotateMessage(messageInstance, 'user');
        }
      }
      
      // Final fallback
      console.warn(`[SupervisorChatClient] Unknown message format at ${idx}:`, m);
      const fallbackMessage = new HumanMessage(String(m));
      return annotateMessage(fallbackMessage, 'user');
    });
  ...
  async getActiveSessions(): Promise<string[]> {
    try {
      // For local deployment, we don't track cross-service sessions
      // Each agent manages its own state through MemorySaver
      return [];
    } catch (error) {
